...
const JapaneseLibraryScreen: React.FC<JapaneseLibraryScreenProps> = ({ onBack, onOpenMaterial }) => {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      setMaterials(await getAllJpMaterials());
      setError('');
    } catch (loadError) {
  ...
  const handleDuplicateChoice = async (choice: 'copy' | 'replace') => {
    ...
    setError('');
    try {
      await finishSave(duplicate.material, choice === 'replace'
        ? { replaceId: duplicate.existing.id }
        : { duplicateTitle: true });
    } catch (saveError) {
    ...
    <main className="jp-library" data-testid="jp-library">
              ...
              {duplicate ? (
                <div className="jp-duplicate-panel">
                  ...
                  <div>
                    ...
                    <button type="button" className="jp-button jp-button--secondary" disabled={isImporting} onClick={() => handleDuplicateChoice('copy')}>コピーとして保存</button>
                    <button type="button" className="jp-button jp-button--primary" disabled={isImporting} onClick={() => handleDuplicateChoice('replace')}>上書きする</button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  {importTab === 'json-file' && (
                    <div className="jp-file-picker">
                      <input ref={fileInputRef} data-testid="jp-json-file" type="file" accept=".json,application/json,text/plain" onChange={handleJsonFile} />
                      <button type="button" className="jp-button jp-button--secondary" onClick={() => fileInputRef.current?.click()}>JSONファイルを選ぶ</button>
                  ...
                  {importTab === 'plain-text' && (
                    <div className="jp-import-form">
                      ...
                      <div className="jp-import-form__row">
                        <label className="jp-field"><span>学習レベル</span><select value={targetLevel} onChange={event => setTargetLevel(event.target.value as JpTargetLevel)}><option value="N4">N4</option><option value="N3">N3</option><option value="N2">N2</option></select></label>
      ...
      {deleteTarget && (
        <div className="jp-modal-backdrop" onClick={() => setDeleteTarget(null)}>
          <section className="jp-confirm-modal" role="alertdialog" aria-modal="true" aria-labelledby="jp-delete-title" onClick={event => event.stopPropagation()}>
            <h2 id="jp-delete-title">この教材を削除しますか？</h2>
            <p>「{deleteTarget.title}」と、その教材の学習記録・登録単語を削除します。</p>
            <div><button type="button" className="jp-button jp-button--quiet" onClick={() => setDeleteTarget(null)}>キャンセル</button><button type="button" className="jp-button jp-button--danger" onClick={handleDelete}>削除する</button></div>
          </section>
        </div>
      )}
    </main>
